...
# ══════════════════════════════════════════════════════════════════════════════
# ESTRATEGIA: KINETIC MOMENTUM VALIDATOR (ID 33)
# ══════════════════════════════════════════════════════════════════════════════
...
class StrategyKineticMomentumValidator(EstrategiaBase):
    ...
    - Optimización: Muestreo dependiente (Slow siempre > Fast).
    """
    ...
    def suggest_params(self, trial: Any) -> Dict[str, Any]:
        """
        CONFIGURACIÓN PROFESIONAL OPTUNA (TPE FRIENDLY)
        Usamos 'Delta Estructural' para forzar la condición Fast < Slow sin romper el optimizador.
        """
        
        # 1. ZLEMA RÁPIDA (Rango Estático)
        # Definimos el rango de la media rápida independientemente.
        fast_len = trial.suggest_int("zlema_fast_len", 20, 500, step=5)
        
        # 2. LA CONDICIÓN (Delta / Separación)
        # Aquí está el truco: Optuna optimiza la 'separación' entre medias.
        # Al ser un rango fijo (10 a 500), Optuna es feliz y no da warnings.
        # El valor mínimo de 10 asegura que nunca sean iguales.
        delta = trial.suggest_int("ma_delta", 10, 1000, step=10)
        
        # 3. CÁLCULO DERIVADO (La Condición Aplicada)
        # Zlema Slow es resultado de la rápida + la separación.
        # Esto garantiza matemáticamente que slow_len > fast_len.
        slow_len = fast_len + delta

        return {
            "zlema_fast_len": fast_len,
            "zlema_slow_len": slow_len, # Pasamos el valor final calculado
            
            # --- FILTRO DE MOMENTUM ---
            "lookbar": trial.suggest_int("lookbar", 10, 100, step=5),
            "req_dist_pct": trial.suggest_float("req_dist_pct", 0.3, 3.0, step=0.1),
        }
