...
"""
Strategy 6 — Z-Score Cross + ADX Filter

Estrategia 100% determinista basada en:
- Cruce de Z-Score (reversión desde extremo)
- Filtro ADX (solo opera en mercado lateral, sin tendencia fuerte)

LÓGICA DE ENTRADA:

  LONG:
    1) Z-Score cruza hacia arriba desde zona extrema negativa
       (z_score[t-1] <= -z_threshold) Y (z_score[t] > -z_threshold)
    2) ADX < adx_max (mercado sin tendencia fuerte)

  SHORT:
    1) Z-Score cruza hacia abajo desde zona extrema positiva
       (z_score[t-1] >= +z_threshold) Y (z_score[t] < +z_threshold)
    2) ADX < adx_max (mercado sin tendencia fuerte)
...
Combina reversión estadística con filtro de régimen de mercado.
"""
...
from modelox.strategies.indicator_specs import cfg_zscore, cfg_adx_numba, cfg_atr


class Strategy6ZScoreCrossAdxFilter:
    """Z-Score Cross + ADX Filter con trailing stop."""

    # Identificación única
    combinacion_id = 6
    name = "ZScore_Cross_ADX_Filter"

    # Indicadores para el plot
    __indicators_used = ["zscore", "adx", "atr"]

    @classmethod
    def get_indicators_used(cls):
        return cls.__indicators_used

    # Espacio de parámetros Optuna (simétrico y con rangos solicitados)
    parametros_optuna = {
        # Z-Score (simétrico)
        "z_length": (10, 30, 5),                   # ventana Z-Score
        "z_threshold": (1.0, 2.5, 0.1),            # umbral de cruce (±)
        # ADX Filter
        "adx_period": (12, 16, 1),                 # período ADX
        "adx_max": (20.0, 40.0, 2.0),              # ADX máximo (mercado lateral)
        # Trailing & SL
        "atr_period": (10, 20, 2),                 # período ATR
        "trailing_atr_mult": (1.0, 4.0, 0.2),      # múltiplo ATR para trailing
        "emergency_sl_pct": (3.0, 12.0, 0.5),      # SL de emergencia (%)
    }
    ...
    def suggest_params(self, trial: Any) -> Dict[str, Any]:
        ...
        z_threshold = trial.suggest_float("z_threshold", 1.0, 2.0, step=0.1)
        adx_period = trial.suggest_int("adx_period", 10, 20, step=2)
        adx_max = trial.suggest_float("adx_max", 20.0, 40.0, step=2.0)
    ...
    def generate_signals(self, df: pl.DataFrame, params: Dict[str, Any]) -> pl.DataFrame:
        ...
        # Warm-up robusto
        max_win = max(z_length, adx_period, atr_period)
        ...
        ind_config = {
            ...
            "adx": cfg_adx_numba(
                ...
                out_adx="adx",
                out_plus_di="plus_di",
                out_minus_di="minus_di"
            ),
            "atr": cfg_atr(period=atr_period, out="atr"),
        }
        df = IndicadorFactory.procesar(df, ind_config)

        # 2) Detectar cruces con filtro ADX usando Numba
        z_arr = df["zscore"].to_numpy().astype(np.float64)
        adx_arr = df["adx"].to_numpy().astype(np.float64)

        signal_long, signal_short = _detect_cross_adx_signals(
            z_arr,
            adx_arr,
            float(z_threshold),
            float(adx_max),
...
def _detect_cross_adx_signals(
    z: np.ndarray,
    adx: np.ndarray,
    z_threshold: float,
    adx_max: float,
) -> tuple:
    """Detecta cruces de Z-Score con filtro ADX.

    LONG: 
      - z cruza hacia arriba desde -z_threshold
      - (z[t-1] <= -z_threshold) AND (z[t] > -z_threshold)
      - ADX < adx_max

    SHORT:
      - z cruza hacia abajo desde +z_threshold
      - (z[t-1] >= +z_threshold) AND (z[t] < +z_threshold)
      - ADX < adx_max
    """
    ...
    for i in range(1, n):
        ...
        if np.isnan(z_prev) or np.isnan(z_curr) or np.isnan(adx_curr):
            continue

        # Filtro ADX: solo opera si ADX < adx_max (mercado lateral)
        if adx_curr >= adx_max:
            continue

        # LONG: cruce hacia arriba desde zona extrema negativa
        # z_prev <= -z_threshold (estaba en zona extrema)
        # z_curr > -z_threshold (cruza hacia arriba, saliendo de extremo)
        if z_prev <= -z_threshold and z_curr > -z_threshold:
            signal_long[i] = True

        # SHORT: cruce hacia abajo desde zona extrema positiva
        # z_prev >= +z_threshold (estaba en zona extrema)
        # z_curr < +z_threshold (cruza hacia abajo, saliendo de extremo)
        if z_prev >= z_threshold and z_curr < z_threshold:
            signal_short[i] = True
