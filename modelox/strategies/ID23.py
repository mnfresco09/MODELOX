...
class EstrategiaID23ZTREND(EstrategiaBase):
    ...
    def generate_signals(self, df: pl.DataFrame, params: Dict[str, Any]) -> pl.DataFrame:
        ...
        def _adx_batch(s: pl.Series) -> pl.Series:
            ...
            n_tot = len(arr)
            high_arr  = np.array([r[0] for r in arr], dtype=np.float64)
            low_arr   = np.array([r[1] for r in arr], dtype=np.float64)
            close_arr = np.array([r[2] for r in arr], dtype=np.float64)
