...
DEFAULT_EXIT_TYPE = "pnl_trailing"

# Parámetros en términos de PNL_PCT (ROI % del trade)
DEFAULT_EXIT_SL_PCT = 8.0       # Salir si PNL_PCT <= -6% (pérdida)
DEFAULT_EXIT_TP_PCT = 14.0       # Salir si PNL_PCT >= +20% (ganancia)
...
DEFAULT_EXIT_SL_PCT_RANGE = (1.0, 50.0, 1.0)      # SL: 5% a 30%
DEFAULT_EXIT_TP_PCT_RANGE = (20.0, 40.0, 1.0)     # TP: 5% a 100%
DEFAULT_EXIT_TRAIL_ACT_PCT_RANGE = (1.0, 50.0, 1.0)   # Activación: 5% a 100%
DEFAULT_EXIT_TRAIL_DIST_PCT_RANGE = (0.5, 20.0, 0.5)  # Distancia: 2.5% a 50%
...
# =============================================================================
# HELPER: NORMALIZACIÓN DE VALORES DE EXIT
# =============================================================================
...
def resolve_exit_settings_for_trial(*, trial: Any, config: Any) -> ExitSettings:
    ...
    # En este proyecto:
    # - pnl_fixed: SL + TP fijos
    # - pnl_trailing: SL fijo + trailing activation/distance (SIN TP)
    # Para trailing, tratamos tp_pct=0.0 como "TP desactivado".
    ...
    if optimize_exits:
        ...
        # TP solo si es fixed (o all por compat)
    ...
    # Usar helper centralizado para normalización
...
def decide_exit_pnl_trailing(
    *,
    ...
    tp_pct_enabled = float(settings.tp_pct) > 0.0
    tp_eur = stk * (abs(float(settings.tp_pct)) / 100.0) if tp_pct_enabled else 0.0
    ...
    if s == "LONG":
        ...
        tp_price = float(entry_price) + (tp_eur / q) if tp_pct_enabled else None
    ...
    for j in range(e + 1, end_idx + 1):
        ...
        if s == "LONG":
            ...
            # 4) TP fijo (DESACTIVADO en pnl_trailing en este proyecto)
            if tp_pct_enabled and tp_price is not None:
                if h_j >= float(tp_price):
...
def trace_exit_pnl_trailing(
    *,
    ...
    if s == "LONG":
        ...
        tp_price = ep + (tp_eur / q) if tp_pct_enabled else None
    ...
    for j in range(e + 1, end_idx + 1):
        ...
        if hit_reason is None:
            ...
            # --- 4) TP fijo (DESACTIVADO en pnl_trailing en este proyecto) ---
            if hit_reason is None and tp_pct_enabled and tp_price is not None:
                if s == "LONG":
                    if h_j >= float(tp_price):
                        hit_reason = "TP"
...
def decide_exit_for_trade(
    ...
    **kwargs: Any,
    ...
    """
    def _coerce_exit_decision(obj: Any) -> Optional[ExitDecision]:
        if obj is None:
            return None
        if isinstance(obj, ExitDecision):
            return obj
        if isinstance(obj, dict):
            if "exit_idx" not in obj:
                return None
            return ExitDecision(
                exit_idx=int(obj.get("exit_idx")),
                reason=str(obj.get("reason", "")),
                exit_price=(float(obj["exit_price"]) if obj.get("exit_price") is not None else None),
            )
        # duck-typing: objetos con atributos exit_idx/reason/exit_price
    ...
    if callable(decide_exit):
        ...
        out = _coerce_exit_decision(out_any)
        if out is not None:
