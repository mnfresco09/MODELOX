...
try:
    ...
    VECINDARIO_MIN_PROFIT_FACTOR = 0.8
    VECINDARIO_MIN_SHARPE = 0.17
...
ROI_SCALE = 300.0       # ROI 52% → ~80%, ROI 120% → ~95%
...
def check_robustness_requirements(metrics: Mapping[str, Any], cfg: Optional[NeighborhoodConfig] = None) -> Tuple[bool, str]:
    ...
    sharpe = _get(metrics, "sharpe", 0.0)
    sharpe_display = sharpe * 10  # Mismo valor que se muestra en PERFORMANCE
    ...
    if sharpe_display < cfg.min_sharpe:
        return False, f"Sharpe ({sharpe_display:.2f}) < {cfg.min_sharpe}"
...
def score_quality_only(metrics: Mapping[str, Any]) -> float:
    """Score sin robustez (máximo ~600 debido al techo de cristal)."""
...
# =============================================================================
# COMPATIBILIDAD CON SCORING v3.0 (LEGACY)
# =============================================================================

@dataclass
class ScoringConfig:
    """Legacy: Configuración del sistema de scoring (ya no se usa en v8.0)."""
    # Mantener para compatibilidad con imports existentes
    pass


def set_study_for_scorer(study: Any) -> None:
    """Legacy: Ya no se necesita en v8.0 - el scoring no depende del estudio."""
    pass


def cleanup_scoring_resources() -> None:
    """Legacy: Ya no hay recursos que limpiar en v8.0."""
    pass
