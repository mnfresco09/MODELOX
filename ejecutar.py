...
from general.configuracion import (
    ACTIVO, ARCHIVO_DATA, COMBINACION_A_EJECUTAR, CONFIG,
...
def main() -> None:
    ...
    cfg = BacktestConfig(
        ...
        qty_max_activo=float(CONFIG.get("QTY_MAX_ACTIVO", float("inf"))),
    )
    # Debug: show resolved quantity limits for the chosen asset
    print(f"[CONFIG] ACTIVO={ACTIVO} QTY_MAX_ACTIVO={cfg.qty_max_activo}")
    ...
    for strat_id in lista_ids:
        ...
        for strategy in strategies:
            ...
            reporters = [
                ...
                CSVReporter(trades_base_dir="resultados/csv", max_archivos=int(MAX_ARCHIVOS_GUARDAR))
            ]
            
            if USAR_EXCEL:
                reporters.append(ExcelReporter(resumen_path="resultados/excel/resumen_trials.xlsx", max_archivos=int(MAX_ARCHIVOS_GUARDAR)))
            
            if GENERAR_PLOTS:
                reporters.append(PlotReporter(plot_base="resultados/plots", fecha_inicio_plot=FECHA_INICIO_PLOT,
            ...
            try:
                ...
                # [MAC-OPTIMIZED] Limpieza agresiva post-estrategia
                del runner
                del reporters
                # Forzamos borrado de referencias circulares
                HealthGuard.force_cleanup()
